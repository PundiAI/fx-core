...
func (suite *rpcTestSuite) TestClient_Tx() {
	...
	for i := 0; i < len(clients); i++ {
		...
		suite.NoError(err)
		suite.Equal(authtypes.NewBaseAccount(toAddress, nil, uint64(14+i+len(suite.network.Validators)), 0), account)
	}
	...
	for i := 0; i < len(clients); i++ {
		...
		suite.NoError(err)
		suite.Equal(authtypes.NewBaseAccount(ethAddress, nil, uint64(16+len(suite.network.Validators)), 0), account)
	}
