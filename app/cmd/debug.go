...
$ %s debug addr 0xA588C66983a81e800Db4dF74564F09f91c026351`, version.AppName, version.AppName),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			valPrefix := prefix + sdk.PrefixValidator + sdk.PrefixOperator
			var addr []byte
			switch {
			case common.IsHexAddress(addrString):
				addr = common.HexToAddress(addrString).Bytes()
			case strings.HasPrefix(addrString, prefix), strings.HasPrefix(addrString, valPrefix):
				_, bz, err := bech32.DecodeAndConvert(addrString)
				if err != nil {
					return fmt.Errorf("decode fxcore address error %v", err)
				}
				addr = bz
			default:
				return fmt.Errorf("expected a valid hex or bech32 address (acc prefix %s), got '%s'", prefix, addrString)
			}
			acc, err := bech32.ConvertAndEncode(prefix, addr)
			if err != nil {
				return fmt.Errorf("convert and encode address error %v", err)
			}
			val, err := bech32.ConvertAndEncode(valPrefix, addr)
			if err != nil {
				return fmt.Errorf("convert and encode val address error %v", err)
			}
	...
	cmd.Flags().String("prefix", fxcore.AddressPrefix, "fxcore address prefix")
	return cmd
}
