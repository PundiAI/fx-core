...
func New(logger log.Logger, db dbm.DB, traceStore io.Writer, loadLatest bool, skipUpgradeHeights map[int64]bool,
	...
	myApp.mm = module.NewManager(
		...
		bsc.NewAppModule(myApp.BscKeeper, myApp.StakingKeeper, myApp.legacyAmino, keys[paramstypes.StoreKey]),
		polygon.NewAppModule(myApp.PolygonKeeper, myApp.StakingKeeper, myApp.legacyAmino, keys[paramstypes.StoreKey]),
		tron.NewAppModule(myApp.TronKeeper, myApp.StakingKeeper, myApp.legacyAmino, keys[paramstypes.StoreKey]),
