...
var (
	appLog = log.NewNopLogger()
)

func SetAppLog(newLogger log.Logger) {
	appLog = newLogger
}
...
func setup(withGenesis bool, invCheckPeriod uint) (*app.App, app.AppGenesisState) {
	db := dbm.NewMemDB()
...
func SetupWithGenesisValSet(t *testing.T, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *app.App {
	...
	totalSupply := bankGenesis.Supply
