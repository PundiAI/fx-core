...
`,
	...
	cmd.Flags().String(flagGRPCAddress, fxconfig.DefaultGRPCAddress, "the gRPC server address to listen on")
	...
	cmd.Flags().StringSlice(JSONRPCAPI, fxconfig.GetDefaultAPINamespaces(), "Defines a list of JSON-RPC namespaces that should be enabled")
	cmd.Flags().String(JSONRPCAddress, fxconfig.DefaultJSONRPCAddress, "the JSON-RPC server address to listen on")
	cmd.Flags().String(JSONWsAddress, fxconfig.DefaultJSONRPCWsAddress, "the JSON-RPC WS server address to listen on")
	cmd.Flags().Uint64(JSONRPCGasCap, fxconfig.DefaultGasCap, "Sets a cap on gas that can be used in eth_call/estimateGas unit is FX (0=infinite)")
	cmd.Flags().Float64(JSONRPCTxFeeCap, fxconfig.DefaultTxFeeCap, "Sets a cap on transaction fee that can be sent via the RPC APIs (1 = default 1 photon)")
	cmd.Flags().Int32(JSONRPCFilterCap, fxconfig.DefaultFilterCap, "Sets the global cap for total number of filters that can be created")
	cmd.Flags().Duration(JSONRPCEVMTimeout, fxconfig.DefaultEVMTimeout, "Sets a timeout used for eth_call (0=infinite)")
	cmd.Flags().Duration(JSONRPCHTTPTimeout, fxconfig.DefaultHTTPTimeout, "Sets a read/write timeout for json-rpc http server (0=infinite)")
	cmd.Flags().Duration(JSONRPCHTTPIdleTimeout, fxconfig.DefaultHTTPIdleTimeout, "Sets a idle timeout for json-rpc http server (0=infinite)")
	cmd.Flags().Int32(JSONRPCLogsCap, fxconfig.DefaultLogsCap, "Sets the max number of results can be returned from single `eth_getLogs` query")
	cmd.Flags().Int32(JSONRPCBlockRangeCap, fxconfig.DefaultBlockRangeCap, "Sets the max block range allowed for `eth_getLogs` query")

	cmd.Flags().String(EVMTracer, fxconfig.DefaultEVMTracer, "the EVM tracer type to collect execution traces from the EVM transaction execution (json|struct|access_list|markdown)")
	cmd.Flags().Uint64(EVMMaxTxGasWanted, fxconfig.DefaultMaxTxGasWanted, "the gas wanted for each eth tx returned in ante handler in check tx mode")
...
func startInProcess(ctx *server.Context, clientCtx client.Context, appCreator types.AppCreator) (err error) {
	...
	config := fxconfig.GetConfig(ctx.Viper)
