...
func (cc *ContractCall) ERC20Burn(amount *big.Int) error {
	data, err := contract.GetFIP20().ABI.Pack("burn", cc.caller.Address(), amount)
	if err != nil {
...
func (cc *ContractCall) ERC20TransferFrom(from, to common.Address, amount *big.Int) error {
	data, err := contract.GetFIP20().ABI.Pack("transferFrom", from, to, amount)
	if err != nil {
	...
	if err := contract.GetFIP20().ABI.UnpackIntoInterface(&unpackedRet, "transferFrom", ret); err != nil {
...
func (cc *ContractCall) ERC20TotalSupply() (*big.Int, error) {
	data, err := contract.GetFIP20().ABI.Pack("totalSupply")
	if err != nil {
	...
	if err := contract.GetFIP20().ABI.UnpackIntoInterface(&unpackedRet, "totalSupply", ret); err != nil {
