...
func TestBridgeCoinAmountABI(t *testing.T) {
	...
	method := crosschainABI.Methods[types.BridgeCoinAmountMethodName]
	require.Equal(t, method, types.BridgeCoinAmountMethod)
	require.Equal(t, 2, len(types.BridgeCoinAmountMethod.Inputs))
	require.Equal(t, 1, len(types.BridgeCoinAmountMethod.Outputs))
}
...
func (suite *PrecompileTestSuite) TestBridgeCoinAmount() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			prepare: prepareFunc,
			malleate: func(token common.Address, target string) ([]byte, []string) {
				pack, err := types.GetABI().Pack(types.BridgeCoinAmountMethodName, token, fxtypes.MustStrToByte32(target))
				suite.Require().NoError(err)
				return pack, nil
			},
			result: true,
		},
		{
			...
			prepare: prepareFunc,
			malleate: func(token common.Address, target string) ([]byte, []string) {
				pack, err := types.GetABI().Pack(types.BridgeCoinAmountMethodName, token, fxtypes.MustStrToByte32(""))
				suite.Require().NoError(err)
				return pack, nil
			},
			error: func(args []string) string {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			tx, err := suite.PackEthereumTx(signer, types.GetAddress(), big.NewInt(0), packData)
			...
			if tc.result {
				...
				unpack, err := types.BridgeCoinAmountMethod.Outputs.Unpack(res.Ret)
				suite.Require().NoError(err)
