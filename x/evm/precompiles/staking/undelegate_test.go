...
func (suite *PrecompileTestSuite) TestUndelegates() {
	testCases := []struct {
	...
	}{
		{
			name: "ok",
			malleate: func(val sdk.ValAddress, shares sdk.Dec) (*evmtypes.MsgEthereumTx, []string) {
				pack, err := fxtypes.MustABIJson(StakingTestABI).Pack(staking.UndelegateMethodName, val.String(), shares.TruncateInt().BigInt())
		...
		},
		{
			...
			malleate: func(val sdk.ValAddress, shares sdk.Dec) (*evmtypes.MsgEthereumTx, []string) {
				newVal := val.String() + "1"
				pack, err := fxtypes.MustABIJson(StakingTestABI).Pack(staking.UndelegateMethodName, newVal, shares.TruncateInt().BigInt())
				suite.Require().NoError(err)
				return suite.PackEthereumTx(suite.signer, suite.precompileStaking, big.NewInt(0), pack), []string{newVal}
			},
			error: func(errArgs []string) string {
				return fmt.Sprintf("undelegate failed: invalid validator address: %s", errArgs[0])
			},
			result: false,
		},
		{
			...
			error: func(errArgs []string) string {
				return fmt.Sprintf("undelegate failed: validator not found: %s", errArgs[0])
			},
			result: false,
		},
	}
