...
func (suite *KeeperTestSuite) TestGetHashFn() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			hash := suite.app.EvmKeeperV0.GetHashFn()(tc.height)
...
func (suite *KeeperTestSuite) TestGetCoinbaseAddress() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			coinbase, err := suite.app.EvmKeeperV0.GetCoinbaseAddress(suite.ctx)
			if tc.expPass {
				suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestGetEthIntrinsicGas() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			ethCfg := params.ChainConfig.EthereumConfig(suite.app.EvmKeeperV0.ChainID())
			ethCfg.HomesteadBlock = big.NewInt(2)
			ethCfg.IstanbulBlock = big.NewInt(3)
			signer := ethtypes.LatestSignerForChainID(suite.app.EvmKeeperV0.ChainID())

			suite.ctx = suite.ctx.WithBlockHeight(tc.height)
			suite.app.EvmKeeperV0.WithContext(suite.ctx)

			m, err := newNativeMessage(
				suite.app.EvmKeeperV0.GetNonce(suite.address),
			...
			gas, err := suite.app.EvmKeeperV0.GetEthIntrinsicGas(m, ethCfg, tc.isContractCreation)
			if tc.noError {
				suite.Require().NoError(err)
			} else {
				suite.Require().Error(err)
...
func (suite *KeeperTestSuite) TestGasToRefund() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.SetupTest() // reset
			suite.app.EvmKeeperV0.AddRefund(10)

			if tc.expPanic {
				panicF := func() {
					suite.app.EvmKeeperV0.GasToRefund(tc.gasconsumed, tc.refundQuotient)
				}
				suite.Require().Panics(panicF)
			} else {
				gr := suite.app.EvmKeeperV0.GasToRefund(tc.gasconsumed, tc.refundQuotient)
...
func (suite *KeeperTestSuite) TestRefundGas() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			keeperParams := suite.app.EvmKeeperV0.GetParams(suite.ctx)
			ethCfg := keeperParams.ChainConfig.EthereumConfig(suite.app.EvmKeeperV0.ChainID())
			signer := ethtypes.LatestSignerForChainID(suite.app.EvmKeeperV0.ChainID())
			...
			suite.app.EvmKeeperV0.AddRefund(params.TxGas)

			if tc.leftoverGas > m.Gas() {
				return
			}
			gasUsed := m.Gas() - tc.leftoverGas
			refund := suite.app.EvmKeeperV0.GasToRefund(gasUsed, tc.refundQuotient)
			suite.Require().Equal(tc.expGasRefund, refund)

			err = suite.app.EvmKeeperV0.RefundGas(m, refund, "FX")
			if tc.noError {
				suite.Require().NoError(err)
			} else {
				suite.Require().Error(err)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			panicF := func() {
				...
				suite.app.EvmKeeperV0.ResetGasMeterAndConsumeGas(tc.gasUsed)
			}
...
func (suite *KeeperTestSuite) TestEVMConfig() {
	suite.SetupTest()
	cfg, err := suite.app.EvmKeeperV0.EVMConfig(suite.ctx)
	suite.Require().NoError(err)
