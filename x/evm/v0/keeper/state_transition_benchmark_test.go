...
func BenchmarkApplyTransaction(b *testing.B) {
	...
	ethSigner := ethtypes.LatestSignerForChainID(suite.app.EvmKeeperV0.ChainID())
	...
	for i := 0; i < b.N; i++ {
		...
		b.StartTimer()
		resp, err := suite.app.EvmKeeperV0.ApplyTransaction(tx)
		b.StopTimer()
...
func BenchmarkApplyMessage(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		b.StartTimer()
		resp, err := suite.app.EvmKeeperV0.ApplyMessage(m, nil, true)
		b.StopTimer()
