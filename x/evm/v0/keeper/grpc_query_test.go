	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.app.EvmKeeperV0.SetState(suite.address, key, value)
				req = &types.QueryStorageRequest{
					Address: suite.address.String(),
					Key:     key.String(),
				}
...
func (suite *KeeperTestSuite) TestQueryCode() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				expCode = []byte("code")
				suite.app.EvmKeeperV0.SetCode(suite.address, expCode)
...
func (suite *KeeperTestSuite) TestQueryTxLogs() {
		...
		},
		{
			...
			func() {
				...
				for _, log := range types.LogsToEthereum(expLogs) {
					suite.app.EvmKeeperV0.AddLog(log)
				}
			},
		},
	}
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			tc.malleate()
			logs := suite.app.EvmKeeperV0.GetTxLogsTransient(txHash)
