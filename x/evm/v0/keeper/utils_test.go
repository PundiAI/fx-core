...
func (suite *KeeperTestSuite) TestCheckSenderBalance() {
	...
	suite.app.EvmKeeperV0.AddBalance(suite.address, hundredInt.BigInt())
	balance := suite.app.EvmKeeperV0.GetBalance(suite.address)
	...
	for i, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			err := EvmKeeperV0.CheckSenderBalance(
				suite.app.EvmKeeperV0.Ctx(),
	...
	for i, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if suite.dynamicTxFee {
				...
				suite.app.EvmKeeperV0.AddBalance(suite.address, hundredBaseFeeInt.BigInt())
				balance := suite.app.EvmKeeperV0.GetBalance(suite.address)
			...
			fees, err := suite.app.EvmKeeperV0.DeductTxCostsFromUserBalance(
				suite.app.EvmKeeperV0.Ctx(),
				*tx,
				txData,
				evmtypes.DefaultEVMDenom,
			...
			if tc.expectPass {
				suite.Require().NoError(err, "valid test %d failed", i)
				if tc.dynamicTxFee {
					baseFee := suite.app.FeeMarketKeeperV0.GetBaseFee(suite.ctx)
