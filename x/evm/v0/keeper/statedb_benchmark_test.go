...
func BenchmarkCreateAccountNew(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		b.StartTimer()
		suite.app.EvmKeeperV0.CreateAccount(addr)
	}
}
...
func BenchmarkCreateAccountExisting(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		suite.app.EvmKeeperV0.CreateAccount(suite.address)
	}
}
...
func BenchmarkAddBalance(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		suite.app.EvmKeeperV0.AddBalance(suite.address, amt)
	}
}
...
func BenchmarkSetCode(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		suite.app.EvmKeeperV0.SetCode(suite.address, hash)
	}
}
...
func BenchmarkSnapshot(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		target := suite.app.EvmKeeperV0.Snapshot()
		require.Equal(b, i, target)
	}

	for i := b.N - 1; i >= 0; i-- {
		require.NotPanics(b, func() {
			suite.app.EvmKeeperV0.RevertToSnapshot(i)
		})
	}
}
...
func BenchmarkSubBalance(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		suite.app.EvmKeeperV0.SubBalance(suite.address, amt)
	}
}
...
func BenchmarkAddRefund(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		suite.app.EvmKeeperV0.AddRefund(1)
	}
}
...
func BenchmarkSuicide(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		suite.app.EvmKeeperV0.Suicide(addr)
	}
}
