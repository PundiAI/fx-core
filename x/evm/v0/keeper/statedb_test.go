...
func (suite *KeeperTestSuite) TestCreateAccount() {
	testCases := []struct {
	...
	}{
		{
			...
			func(addr common.Address) {
				suite.app.EvmKeeperV0.AddBalance(addr, big.NewInt(100))
				suite.Require().NotZero(suite.app.EvmKeeperV0.GetBalance(addr).Int64())
			},
			func(addr common.Address) {
				suite.Require().Equal(suite.app.EvmKeeperV0.GetBalance(addr).Int64(), int64(100))
			},
		},
		{
			"create account",
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			tc.malleate(tc.addr)
			suite.app.EvmKeeperV0.CreateAccount(tc.addr)
			tc.callback(tc.addr)
		})
	}
}
...
func (suite *KeeperTestSuite) TestAddBalance() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			prev := suite.app.EvmKeeperV0.GetBalance(suite.address)
			suite.app.EvmKeeperV0.AddBalance(suite.address, tc.amount)
			post := suite.app.EvmKeeperV0.GetBalance(suite.address)
...
func (suite *KeeperTestSuite) TestSubBalance() {
	testCases := []struct {
		...
		},
		{
			...
			big.NewInt(50),
			func() {
				suite.app.EvmKeeperV0.AddBalance(suite.address, big.NewInt(100))
			},
			false,
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			prev := suite.app.EvmKeeperV0.GetBalance(suite.address)
			suite.app.EvmKeeperV0.SubBalance(suite.address, tc.amount)
			post := suite.app.EvmKeeperV0.GetBalance(suite.address)
...
func (suite *KeeperTestSuite) TestGetNonce() {
	testCases := []struct {
		...
		},
		{
			...
			1,
			func() {
				suite.app.EvmKeeperV0.SetNonce(suite.address, 1)
			},
		},
	}
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			nonce := suite.app.EvmKeeperV0.GetNonce(tc.address)
...
func (suite *KeeperTestSuite) TestSetNonce() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			suite.app.EvmKeeperV0.SetNonce(tc.address, tc.nonce)
			nonce := suite.app.EvmKeeperV0.GetNonce(tc.address)
...
func (suite *KeeperTestSuite) TestGetCodeHash() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.EvmKeeperV0.SetCode(suite.address, []byte("codeHash"))
			},
		},
	}
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			hash := suite.app.EvmKeeperV0.GetCodeHash(tc.address)
...
func (suite *KeeperTestSuite) TestSetCode() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			prev := suite.app.EvmKeeperV0.GetCode(tc.address)
			suite.app.EvmKeeperV0.SetCode(tc.address, tc.code)
			post := suite.app.EvmKeeperV0.GetCode(tc.address)
			...
			suite.Require().Equal(len(post), suite.app.EvmKeeperV0.GetCodeSize(tc.address))

			suite.app.EvmKeeperV0.ClearStateError()
		})
	}
}
...
func (suite *KeeperTestSuite) TestRefund() {
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				suite.app.EvmKeeperV0.AddRefund(11)
			},
			1,
			false,
		},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.expPanic {
				suite.Require().Panics(func() { suite.app.EvmKeeperV0.SubRefund(10) })
			} else {
				suite.app.EvmKeeperV0.SubRefund(10)
				suite.Require().Equal(tc.expRefund, suite.app.EvmKeeperV0.GetRefund())
			}

			// clear and reset refund from store
			suite.app.EvmKeeperV0.ResetRefundTransient(suite.ctx)
			suite.Require().Zero(suite.app.EvmKeeperV0.GetRefund())
		})
	}
}
...
func (suite *KeeperTestSuite) TestState() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			suite.app.EvmKeeperV0.SetState(suite.address, tc.key, tc.value)
			value := suite.app.EvmKeeperV0.GetState(suite.address, tc.key)
...
func (suite *KeeperTestSuite) TestCommittedState() {
	...
	suite.app.EvmKeeperV0.SetState(suite.address, key, value1)

	suite.app.EvmKeeperV0.Snapshot()

	suite.app.EvmKeeperV0.SetState(suite.address, key, value2)
	tmp := suite.app.EvmKeeperV0.GetState(suite.address, key)
	suite.Require().Equal(value2, tmp)
	tmp = suite.app.EvmKeeperV0.GetCommittedState(suite.address, key)
	suite.Require().Equal(value1, tmp)

	suite.app.EvmKeeperV0.CommitCachedContexts()
...
func (suite *KeeperTestSuite) TestSuicide() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			suite.Require().Equal(tc.suicided, suite.app.EvmKeeperV0.Suicide(suite.address))
			suite.Require().Equal(tc.suicided, suite.app.EvmKeeperV0.HasSuicided(suite.address))
		})
	}
}
...
func (suite *KeeperTestSuite) TestExist() {
	testCases := []struct {
		...
		{"success, has suicided", suite.address, func() {
			suite.app.EvmKeeperV0.Suicide(suite.address)
		}, true},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.Require().Equal(tc.exists, suite.app.EvmKeeperV0.Exist(tc.address))
		})
	}
}
...
func (suite *KeeperTestSuite) TestEmpty() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.Require().Equal(tc.empty, suite.app.EvmKeeperV0.Empty(tc.address))
		})
	}
}
...
func (suite *KeeperTestSuite) TestSnapshot() {
	...
	testCases := []struct {
		...
		{"simple revert", func() {
			revision := suite.app.EvmKeeperV0.Snapshot()
			suite.Require().Zero(revision)

			suite.app.EvmKeeperV0.SetState(suite.address, key, value1)
			suite.Require().Equal(value1, suite.app.EvmKeeperV0.GetState(suite.address, key))

			suite.app.EvmKeeperV0.RevertToSnapshot(revision)

			// reverted
			suite.Require().Equal(common.Hash{}, suite.app.EvmKeeperV0.GetState(suite.address, key))
		}},
		{"nested snapshot/revert", func() {
			revision1 := suite.app.EvmKeeperV0.Snapshot()
			suite.Require().Zero(revision1)

			suite.app.EvmKeeperV0.SetState(suite.address, key, value1)

			revision2 := suite.app.EvmKeeperV0.Snapshot()

			suite.app.EvmKeeperV0.SetState(suite.address, key, value2)
			suite.Require().Equal(value2, suite.app.EvmKeeperV0.GetState(suite.address, key))

			suite.app.EvmKeeperV0.RevertToSnapshot(revision2)
			suite.Require().Equal(value1, suite.app.EvmKeeperV0.GetState(suite.address, key))

			suite.app.EvmKeeperV0.RevertToSnapshot(revision1)
			suite.Require().Equal(common.Hash{}, suite.app.EvmKeeperV0.GetState(suite.address, key))
		}},
		{"jump revert", func() {
			revision1 := suite.app.EvmKeeperV0.Snapshot()
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.Require().True(suite.app.EvmKeeperV0.CachedContextsEmpty())
		})
	}
}
...
func (suite *KeeperTestSuite) TestAddLog() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.EvmKeeperV0.SetTxHashTransient(txHash)
				suite.app.EvmKeeperV0.AddLog(&ethtypes.Log{
					Address: addr,
				})
				suite.app.EvmKeeperV0.IncreaseTxIndexTransient()
			},
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.app.EvmKeeperV0.SetTxHashTransient(tc.hash)
			suite.app.EvmKeeperV0.AddLog(tc.log)
			logs := suite.app.EvmKeeperV0.GetTxLogsTransient(tc.hash)
...
func (suite *KeeperTestSuite) TestPrepareAccessList() {
	...
	suite.app.EvmKeeperV0.PrepareAccessList(suite.address, &dest, precompiles, accesses)

	suite.Require().True(suite.app.EvmKeeperV0.AddressInAccessList(suite.address))
	suite.Require().True(suite.app.EvmKeeperV0.AddressInAccessList(dest))

	for _, precompile := range precompiles {
		suite.Require().True(suite.app.EvmKeeperV0.AddressInAccessList(precompile))
	}

	for _, access := range accesses {
		for _, key := range access.StorageKeys {
			addrOK, slotOK := suite.app.EvmKeeperV0.SlotInAccessList(access.Address, key)
...
func (suite *KeeperTestSuite) TestAddAddressToAccessList() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			suite.app.EvmKeeperV0.AddAddressToAccessList(tc.addr)
			addrOk := suite.app.EvmKeeperV0.AddressInAccessList(tc.addr)
...
func (suite *KeeperTestSuite) AddSlotToAccessList() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			suite.app.EvmKeeperV0.AddSlotToAccessList(tc.addr, tc.slot)
			addrOk, slotOk := suite.app.EvmKeeperV0.SlotInAccessList(tc.addr, tc.slot)
			suite.Require().True(addrOk, tc.addr.Hex())
...
func (suite *KeeperTestSuite) TestForEachStorage() {
	...
	testCase := []struct {
	...
	}{
		{
			...
			func() {
				for i := 0; i < 5; i++ {
					suite.app.EvmKeeperV0.SetState(suite.address, common.BytesToHash([]byte(fmt.Sprintf("key%d", i))), common.BytesToHash([]byte(fmt.Sprintf("value%d", i))))
				}
			},
		...
		},
		{
			"filter state",
			func() {
				suite.app.EvmKeeperV0.SetState(suite.address, common.BytesToHash([]byte("key")), common.BytesToHash([]byte("value")))
				suite.app.EvmKeeperV0.SetState(suite.address, common.BytesToHash([]byte("filterkey")), common.BytesToHash([]byte("filtervalue")))
			},
			func(key, value common.Hash) bool {
	...
	for _, tc := range testCase {
		suite.Run(tc.name, func() {
			...
			err := suite.app.EvmKeeperV0.ForEachStorage(suite.address, tc.callback)
			suite.Require().NoError(err)
