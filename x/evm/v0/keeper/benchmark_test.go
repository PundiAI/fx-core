...
func SetupContract(b *testing.B) (*KeeperTestSuite, common.Address) {
	...
	amt := sdk.Coins{sdk.NewCoin(fxtypes.DefaultDenom, sdk.NewInt(1e18))}
...
func DoBenchmark(b *testing.B, txBuilder TxBuilder) {
	...
	err := msg.Sign(ethtypes.LatestSignerForChainID(suite.app.EvmKeeperV0.ChainID()), suite.signer)
	require.NoError(b, err)
	...
	for i := 0; i < b.N; i++ {
		...
		rsp, err := suite.app.EvmKeeperV0.EthereumTx(sdk.WrapSDKContext(ctx), msg)
		require.NoError(b, err)
...
func BenchmarkTokenTransfer(b *testing.B) {
	DoBenchmark(b, func(suite *KeeperTestSuite, contract common.Address) *types.MsgEthereumTx {
		...
		return types.NewTx(suite.app.EvmKeeperV0.ChainID(), nonce, &contract, big.NewInt(0), 410000, big.NewInt(1), nil, nil, input, nil)
	})
}
...
func BenchmarkEmitLogs(b *testing.B) {
	DoBenchmark(b, func(suite *KeeperTestSuite, contract common.Address) *types.MsgEthereumTx {
		...
		require.NoError(b, err)
		nonce := suite.app.EvmKeeperV0.GetNonce(suite.address)
		return types.NewTx(suite.app.EvmKeeperV0.ChainID(), nonce, &contract, big.NewInt(0), 4100000, big.NewInt(1), nil, nil, input, nil)
	})
}
...
func BenchmarkMessageCall(b *testing.B) {
	...
	require.NoError(b, err)
	nonce := suite.app.EvmKeeperV0.GetNonce(suite.address)
	msg := types.NewTx(suite.app.EvmKeeperV0.ChainID(), nonce, &contract, big.NewInt(0), 25000000, big.NewInt(1), nil, nil, input, nil)
