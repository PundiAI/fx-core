...
func (suite *KeeperTestSuite) TestCalculateBaseFee() {
	testCases := []struct {
		...
		},
		{
			...
			big.NewInt(suite.app.FeeMarketKeeperV0.GetParams(suite.ctx).InitialBaseFee),
		},
		{
			...
			func() {
				...
				// Set gas used
				suite.app.FeeMarketKeeperV0.SetBlockGasUsed(suite.ctx, 100)
		...
		},
		{
			...
			func() {
				...
				suite.app.FeeMarketKeeperV0.SetBlockGasUsed(suite.ctx, 200)
		...
		},
		{
			...
			func() {
				...
				suite.app.FeeMarketKeeperV0.SetBlockGasUsed(suite.ctx, 50)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			fee := suite.app.FeeMarketKeeperV0.CalculateBaseFee(suite.ctx)
			if tc.NoBaseFee {
