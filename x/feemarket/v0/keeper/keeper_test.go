...
import (
	...
	"github.com/functionx/fx-core/x/feemarket/v0/keeper"
...
func (suite *KeeperTestSuite) DoSetupTest(t require.TestingT) {
	...
	require.NoError(suite.T(), InitEvmModuleParams(suite.ctx, suite.app.EvmKeeperV0, suite.app.FeeMarketKeeperV0))
	queryHelper := baseapp.NewQueryServerTestHelper(suite.ctx, suite.app.InterfaceRegistry())
	types.RegisterQueryServer(queryHelper, suite.app.FeeMarketKeeperV0)
...
func (suite *KeeperTestSuite) TestSetGetBlockGasUsed() {
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				suite.app.FeeMarketKeeperV0.SetBlockGasUsed(suite.ctx, uint64(1000000))
			},
			uint64(1000000),
		},
	}
	for _, tc := range testCases {
		...
		gas := suite.app.FeeMarketKeeperV0.GetBlockGasUsed(suite.ctx)
...
func (suite *KeeperTestSuite) TestSetGetGasFee() {
	testCases := []struct {
	...
	}{
		{
			"with last block given",
			func() {
				suite.app.FeeMarketKeeperV0.SetBaseFee(suite.ctx, sdk.OneDec().BigInt())
			},
			sdk.OneDec().BigInt(),
		},
	}
	...
	for _, tc := range testCases {
		...
		fee := suite.app.FeeMarketKeeperV0.GetBaseFee(suite.ctx)
