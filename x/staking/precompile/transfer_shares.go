...
func (c *Contract) TransferShares(evm *vm.EVM, contract *vm.Contract, readonly bool) ([]byte, error) {
	if readonly {
	...
	err := stateDB.ExecuteNativeAction(contract.Address(), nil, func(ctx sdk.Context) error {
		...
		result, err = TransferSharesMethod.Outputs.Pack(token, reward)
...
func (c *Contract) TransferFromShares(evm *vm.EVM, contract *vm.Contract, readonly bool) ([]byte, error) {
	if readonly {
	...
	err := stateDB.ExecuteNativeAction(contract.Address(), nil, func(ctx sdk.Context) error {
		...
		result, err = TransferFromSharesMethod.Outputs.Pack(token, reward)
		if err != nil {
			return err
		}
...
func (c *Contract) handlerTransferShares(
	...
	withdrawAddr := c.distrKeeper.GetDelegatorWithdrawAddr(ctx, to.Bytes())
