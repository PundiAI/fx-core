...
func TestStakingDelegationABI(t *testing.T) {
	...
	method := stakingABI.Methods[precompile.DelegationMethod.Name]
	require.Equal(t, method, precompile.DelegationMethod)
	require.Equal(t, 2, len(precompile.DelegationMethod.Inputs))
	require.Equal(t, 2, len(precompile.DelegationMethod.Outputs))
}
...
func (suite *PrecompileTestSuite) TestDelegation() {
	testCases := []struct {
	...
	}{
		{
			name: "ok",
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				pack, err := precompile.GetABI().Pack(precompile.DelegationMethodName, val.String(), del)
		...
		},
		{
			...
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				newVal := val.String() + "1"
				pack, err := precompile.GetABI().Pack(precompile.DelegationMethodName, newVal, del)
				suite.Require().NoError(err)
...

		{
			name: "contract - ok",
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				pack, err := contract.MustABIJson(testscontract.StakingTestMetaData.ABI).Pack(StakingTestDelegationName, val.String(), del)
				suite.Require().NoError(err)
				return pack, nil
			},
			result: true,
		...
		},
		{
			...
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				newVal := val.String() + "1"
				pack, err := contract.MustABIJson(testscontract.StakingTestMetaData.ABI).Pack(StakingTestDelegationName, newVal, del)
				suite.Require().NoError(err)
				return pack, []string{newVal}
			},
			error: func(errArgs []string) string {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			stakingABI := precompile.GetABI()
			delegateFunc := precompile.DelegateMethodName
			delegationFunc := precompile.DelegationMethodName
