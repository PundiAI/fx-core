...
func TestStakingRedelegateABI(t *testing.T) {
	...
	method := stakingABI.Methods[precompile.RedelegateMethodName]
	require.Equal(t, method, precompile.RedelegateMethod)
	require.Equal(t, 3, len(precompile.RedelegateMethod.Inputs))
	require.Equal(t, 3, len(precompile.RedelegateMethod.Outputs))

	event := stakingABI.Events[precompile.RedelegateEventName]
	require.Equal(t, event, precompile.RedelegateEvent)
	require.Equal(t, 6, len(precompile.RedelegateEvent.Inputs))
}
...
func (suite *PrecompileTestSuite) TestRedelegate() {
	mustPackRedelegateFunc := func(valSrc, valDst sdk.ValAddress, shares sdk.Dec) []byte {
		pack, err := precompile.GetABI().Pack(precompile.RedelegateMethodName, valSrc.String(), valDst.String(), shares.TruncateInt().BigInt())
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			redelegateMethodName := precompile.RedelegateMethodName
			...
			if tc.result {
				...
				for _, log := range res.Logs {
					if log.Topics[0] != precompile.RedelegateEvent.ID.String() {
						continue
					}

					suite.Equal(log.Address, precompile.GetAddress().String())
					suite.Equal(log.Topics[1], delAddr.Hash().String())
					unpack, err := precompile.RedelegateEvent.Inputs.NonIndexed().Unpack(log.Data)
					suite.NoError(err)
				...
				for _, event := range suite.ctx.EventManager().Events() {
					...
					for _, attr := range event.Attributes {
						if attr.Key == stakingtypes.AttributeKeySrcValidator {
							suite.Equal(attr.Value, val0.GetOperator().String())
						}
						if attr.Key == stakingtypes.AttributeKeyDstValidator {
							suite.Equal(attr.Value, val1.GetOperator().String())
						}
						if attr.Key == sdk.AttributeKeyAmount {
							suite.Require().Equal(attr.Value, val0.TokensFromShares(delegation0.Shares).TruncateInt().String())
						}
						if attr.Key == stakingtypes.AttributeKeyCompletionTime {
							suite.Require().Equal(attr.Value, redelegates[0].Entries[0].CompletionTime.Format(time.RFC3339))
						}
					}
					existEvent = true
				}
