...
func TestStakingDelegationRewardsABI(t *testing.T) {
	...
	method := stakingABI.Methods[precompile.DelegationRewardsMethod.Name]
	require.Equal(t, method, precompile.DelegationRewardsMethod)
	require.Equal(t, 2, len(precompile.DelegationRewardsMethod.Inputs))
	require.Equal(t, 1, len(precompile.DelegationRewardsMethod.Outputs))
}
...
func (suite *PrecompileTestSuite) TestDelegationRewards() {
	testCases := []struct {
	...
	}{
		{
			name: "ok",
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				pack, err := precompile.GetABI().Pack(precompile.DelegationRewardsMethodName, val.String(), del)
		...
		},
		{
			...
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				newVal := val.String() + "1"
				pack, err := precompile.GetABI().Pack(precompile.DelegationRewardsMethodName, newVal, del)
				suite.Require().NoError(err)
		...
		},
		{
			name: "contract - ok",
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				pack, err := contract.MustABIJson(testscontract.StakingTestMetaData.ABI).Pack(StakingTestDelegationRewardsName, val.String(), del)
				suite.Require().NoError(err)
				return pack, nil
			},
			result: true,
		},
		{
			...
			malleate: func(val sdk.ValAddress, del common.Address) ([]byte, []string) {
				newVal := val.String() + "1"
				pack, err := contract.MustABIJson(testscontract.StakingTestMetaData.ABI).Pack(StakingTestDelegationRewardsName, newVal, del)
				suite.Require().NoError(err)
				return pack, []string{newVal}
			},
			error: func(errArgs []string) string {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			delegationRewardsMethodName := precompile.DelegationRewardsMethodName
			...
			resp, err := suite.DistributionQueryClient(suite.ctx).DelegationRewards(suite.ctx, &distrtypes.QueryDelegationRewardsRequest{DelegatorAddress: sdk.AccAddress(delAddr.Bytes()).String(), ValidatorAddress: val0.GetOperator().String()})
			suite.Require().NoError(err)
