...
func (b *BridgeValidator) ValidateBasic() error {
	...
	if err := ValidateEthAddressAndValidateChecksum(b.EthAddress); err != nil {
...
func (b BridgeValidators) Sort() {
	sort.Slice(b, func(i, j int) bool {
		if b[i].Power == b[j].Power {
			...
			return EthAddrLessThan(b[i].EthAddress, b[j].EthAddress)
		}
...
func (b BridgeValidators) PowerDiff(c BridgeValidators) float64 {
	...
	for _, bv := range b {
		powers[bv.EthAddress] = int64(bv.Power)
	}

	// subtract c powers from powers in the map, initializing
	// uninitialized keys with negative numbers
	for _, bv := range c {
		if val, ok := powers[bv.EthAddress]; ok {
			powers[bv.EthAddress] = val - int64(bv.Power)
		} else {
			powers[bv.EthAddress] = -int64(bv.Power)
		}
	}
...
func NewValset(nonce, height uint64, members BridgeValidators) *Valset {
	...
	return &Valset{Nonce: nonce, Members: mem, Height: height}
}

// GetCheckpoint returns the checkpoint
func (v Valset) GetCheckpoint(gravityIDStr string) []byte {
	...
	for i, m := range v.Members {
		memberAddresses[i] = gethcommon.HexToAddress(m.EthAddress)
