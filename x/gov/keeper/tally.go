...
func (keeper Keeper) Tally(ctx context.Context, proposal v1.Proposal) (passes, burnDeposits bool, tallyResults v1.TallyResult, err error) {
	...
	// get custom quorum params by proposal msg type
	proposalQuorum := keeper.GetCustomMsgQuorum(ctx, params.Quorum, proposal)

	quorum, _ := math.LegacyNewDecFromStr(proposalQuorum)
