	...
	for _, file_info := range files {
		...
		for i, tlaTc := range tlaTestCases {
			...
			registerDenom := func() {
				...
				if !suite.chainB.GetSimApp().FxTransferKeeper.HasDenomTrace(suite.chainB.GetContext(), traceHash) {
					suite.chainB.GetSimApp().FxTransferKeeper.SetDenomTrace(suite.chainB.GetContext(), denomTrace)
				}
			}
			...
			suite.Run(fmt.Sprintf("Case %s", description), func() {
				...
				case "FxSendTransfer":
					...
					if err == nil {
						...
						err = suite.chainB.GetSimApp().FxTransferKeeper.FxSendTransfer(
				...
				case "OnRecvPacket":
					...
					err = suite.chainB.GetSimApp().FxTransferKeeper.OnRecvPacket(suite.chainB.GetContext(), packet, tc.packet.Data)
				case "FxOnRecvPacket":
					if len(tc.packet.Data.GetFee()) == 0 {
						tc.packet.Data.Fee = sdk.ZeroInt().String()
					}
				...
				case "OnTimeoutPacket":
					registerDenom()
					err = suite.chainB.GetSimApp().FxTransferKeeper.OnTimeoutPacket(suite.chainB.GetContext(), packet, tc.packet.Data)
				case "OnRecvAcknowledgementResult":
					err = suite.chainB.GetSimApp().FxTransferKeeper.OnAcknowledgementPacket(
