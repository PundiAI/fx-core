	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				newContractAddr = suite.DeployContract(erc20Name, erc20Symbol, 18)
			},
			false,
		},
		{
		...
		},
		{
			...
			func() {
				...
				metadata := banktypes.Metadata{
					...
					Display:     erc20Symbol,
					Description: erc20Name,
					DenomUnits:  []*banktypes.DenomUnit{{Denom: erc20Name}}}
				suite.app.BankKeeper.SetDenomMetaData(suite.ctx, metadata)
				suite.Commit()
		...
		},
		{
			...
			func() {
				...
				metadata := banktypes.Metadata{
					...
					Description: erc20Name,
					DenomUnits: []*banktypes.DenomUnit{
						{Denom: pair.Denom, Exponent: 0},
						{Denom: erc20Symbol, Exponent: 18},
					},
				}
				suite.app.BankKeeper.SetDenomMetaData(suite.ctx, metadata)
				suite.Commit()
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expPass {
				...
				suite.Require().Equal(erc20Name, metadata.Description)
			} else {
				suite.Require().Error(err, tc.name)
				if suite.app.IntrarelayerKeeper.IsTokenPairRegistered(suite.ctx, pair.GetID()) {
					...
					suite.Require().Equal(erc20Name, metadata.Description, "check metadata")
				}
			}
		})
	}
