...
func (k Keeper) QueryFIP20(ctx sdk.Context, contract common.Address) (types.FIP20Data, error) {
	...
	if !found {
		return types.FIP20Data{}, sdkerrors.Wrap(types.ErrInvalidContract, "fip20 contract not found")
	}

	// Name
	res, err := k.CallEVM(ctx, fip20ABI, types.ModuleAddress, contract, "name")
	if err != nil {
		return types.FIP20Data{}, err
	}

	if err := fip20ABI.UnpackIntoInterface(&nameRes, "name", res.Ret); err != nil {
		return types.FIP20Data{}, sdkerrors.Wrapf(sdkerrors.ErrJSONUnmarshal, "failed to unpack name: %s", err.Error())
	}

	// Symbol
	res, err = k.CallEVM(ctx, fip20ABI, types.ModuleAddress, contract, "symbol")
	if err != nil {
		return types.FIP20Data{}, err
	}

	if err := fip20ABI.UnpackIntoInterface(&symbolRes, "symbol", res.Ret); err != nil {
		return types.FIP20Data{}, sdkerrors.Wrapf(sdkerrors.ErrJSONUnmarshal, "failed to unpack symbol: %s", err.Error())
	}

	// Decimals
	res, err = k.CallEVM(ctx, fip20ABI, types.ModuleAddress, contract, "decimals")
	if err != nil {
		return types.FIP20Data{}, err
	}

	if err := fip20ABI.UnpackIntoInterface(&decimalRes, "decimals", res.Ret); err != nil {
...
func (k Keeper) QueryFIP20BalanceOf(ctx sdk.Context, contract, addr common.Address) (*big.Int, error) {
	fip20ABI, found := contracts.GetABI(ctx.BlockHeight(), contracts.FIP20UpgradeType)
	if !found {
		return nil, sdkerrors.Wrap(types.ErrInvalidContract, "fip20 contract not found")
	}

	res, err := k.CallEVM(ctx, fip20ABI, types.ModuleAddress, contract, "balanceOf", addr)
	if err != nil {
		return nil, err
	}

	var balanceRes types.FIP20Uint256Response
	if err := fip20ABI.UnpackIntoInterface(&balanceRes, "balanceOf", res.Ret); err != nil {
