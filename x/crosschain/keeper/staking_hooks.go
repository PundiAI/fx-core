...
func (h Hooks) AfterValidatorBeginUnbonding(ctx sdk.Context, _ sdk.ConsAddress, valAddr sdk.ValAddress) {
	...
	if found := h.k.HaiOracle(ctx, valAddr.Bytes()); !found {
		return
	}
	oracle, _ := h.k.GetOracle(ctx, valAddr.Bytes())
	if oracle.Jailed {
		return
	}
	oracle.Jailed = true
	oracle.JailedHeight = ctx.BlockHeight()
	h.k.SetOracle(ctx, oracle)
	h.k.SetLastOracleSlashBlockHeight(ctx, uint64(ctx.BlockHeight()))
