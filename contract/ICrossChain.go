...
//
// Solidity: function bridgeCoinAmount(address token, bytes32 target) view returns(uint256 _amount)
func (_ICrossChain *ICrossChainCaller) BridgeCoinAmount(opts *bind.CallOpts, token common.Address, target [32]byte) (*big.Int, error) {
	var out []interface{}
	err := _ICrossChain.contract.Call(opts, &out, "bridgeCoinAmount", token, target)
...
//
// Solidity: function bridgeCoinAmount(address token, bytes32 target) view returns(uint256 _amount)
func (_ICrossChain *ICrossChainSession) BridgeCoinAmount(token common.Address, target [32]byte) (*big.Int, error) {
	return _ICrossChain.Contract.BridgeCoinAmount(&_ICrossChain.CallOpts, token, target)
}

// BridgeCoinAmount is a free data retrieval call binding the contract method 0x8fefb765.
//
// Solidity: function bridgeCoinAmount(address token, bytes32 target) view returns(uint256 _amount)
func (_ICrossChain *ICrossChainCallerSession) BridgeCoinAmount(token common.Address, target [32]byte) (*big.Int, error) {
	return _ICrossChain.Contract.BridgeCoinAmount(&_ICrossChain.CallOpts, token, target)
}
