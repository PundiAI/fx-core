...
subtask(SUB_SEND_ETH, "send eth").setAction(async (taskArgs, hre) => {
  const {
    to,
    value,
    wallet,
...
subtask(SUB_CREATE_TRANSACTION, "create transaction").setAction(
  async (taskArgs, hre) => {
    ...
    if (gasPrice && maxFeePerGas) {
      throw new Error(
        "Please provide only one of gasPrice or maxFeePerGas and maxPriorityFeePerGas"
      );
    }
    ...
    const transaction: TransactionLike = {
      ...
      nonce: nonce
        ? nonce
        : await hre.ethers.provider.getTransactionCount(from),
      gasLimit: gasLimit
        ? gasLimit
...
subtask(
  ...
  if (
    ...
    throw new Error(
      "Please provide only one of private key or ledger or mnemonic"
    );
  }
  if (privateKey) {
...
subtask(SUB_MNEMONIC_WALLET, "mnemonic wallet account").setAction(
  async (taskArgs, hre) => {
    ...
    const wallet = HDNodeWallet.fromPhrase(mnemonic, undefined, _path).connect(
      provider
    );
    return { provider, wallet };
  }
...
export async function GetOracleSet(
  restRpc: string,
  chainName: string
): Promise<OracleSet> {
  const request_string =
    restRpc + `/fx/crosschain/v1/oracle_set/current?chain_name=${chainName}`;
  const response = await axios.get(request_string);
  return response.data.oracle_set;
}

export async function GetGravityId(
  restRpc: string,
  chainName: string
): Promise<string> {
  const request_string =
    restRpc + `/fx/crosschain/v1/params?chain_name=${chainName}`;
  const response = await axios.get(request_string);
