...
const send = task(
  "send",
  "send tx, Example: npx hardhat send 0x... transfer(address,uint256) 0x... 1000000000000000000 --privateKey ..."
)
...
task(
  "call",
  "call contract, Example: npx hardhat call 0x... balanceOf(address)(uint256) 0x..."
)
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    console.log(
      splitByComma(
        abiInterface.decodeFunctionResult(abi.name as string, result).toString()
      )
    );
  });
...
task(
  "encode",
  "encode function data, Example: npx hardhat encode transfer(address,uint256) 0x... 1000000000000000000"
)
  .addVariadicPositionalParam(
    "params",
    "encode function data params",
    undefined,
    string,
    true
  )
...
task(
  "encode-erc20-data",
  "encode asset data, Example: npx hardhat encode-erc20-data 0x... 1000000000000000000"
)
