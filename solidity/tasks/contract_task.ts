...
const migrateBridge = task("migrate-bridge", "migrate bridge")
  ...
  .setAction(async (taskArgs, hre) => {
    ...
    const oldBridgeContract = (await hre.ethers.getContractAt(
      "FxBridgeLogic",
      oldBridge
    )) as FxBridgeLogic;
    const fxBridgeId = await oldBridgeContract
      .connect(provider)
      .state_fxBridgeId();
    const powerThreshold = await oldBridgeContract
      .connect(provider)
      .state_powerThreshold();
    const lastEventNonce = await oldBridgeContract
      .connect(provider)
      .state_lastEventNonce();
    const lastOracleSetNonce = await oldBridgeContract
      .connect(provider)
      .state_lastOracleSetNonce();
    const lastOracleSetCheckpoint = await oldBridgeContract
      .connect(provider)
      .state_lastOracleSetCheckpoint();
    const bridgeTokens = await oldBridgeContract
      .connect(provider)
      .getBridgeTokenList();
    ...
    for (let i = 0; i < bridgeTokens.length; i++) {
      ...
      const batchNonce = await oldBridgeContract
        .connect(provider)
        .state_lastBatchNonces(token.addr);
      lastBatchNonce.push(batchNonce);
      const status = await oldBridgeContract
        .connect(provider)
        .tokenStatus(token.addr);
      tokenStatus.push(status);
    }

    const bridgeMigrateLogicFactory: any = await hre.ethers.getContractFactory(
      "FxBridgeMigrateLogic"
    );
    let proxyFactory: any = await hre.ethers.getContractFactory(
      "TransparentUpgradeableProxy"
    );
    const initData = bridgeMigrateLogicFactory.interface.encodeFunctionData(
      "migrateInit",
      [
        fxBridgeId,
        powerThreshold,
    ...
    if (!newBridgeProxy) {
      const paramData = proxyFactory.interface.encodeDeploy([
        newBridgeLogic,
        admin,
        initData,
      ]);
      ...
      proxyFactory = await hre.ethers.getContractAt(
