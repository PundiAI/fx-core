...
func TestEvmTestSuite(t *testing.T) {
	...
	dial, err := ethclient.Dial(GetWeb3Url())
...
func (suite *EvmTestSuite) AccAddress() sdk.AccAddress {
	return sdk.AccAddress(suite.privateKey.PubKey().Address())
}

func (suite *EvmTestSuite) HexAddress() common.Address {
	return common.BytesToAddress(suite.privateKey.PubKey().Address())
}

func (suite *EvmTestSuite) TransactOpts() *bind.TransactOpts {
	ecdsa, err := crypto.ToECDSA(suite.privateKey.Bytes())
...
func (suite *EvmTestSuite) PendingTx(tx *ethtypes.Transaction) {
...
func (suite *EvmTestSuite) Token(denom string) common.Address {
	pair, err := erc20types.NewQueryClient(suite.grpcClient).TokenPair(context.Background(), &erc20types.QueryTokenPairRequest{Token: denom})
	require.NoError(suite.T(), err)
...
func (suite *EvmTestSuite) Transfer(recipient common.Address, value *big.Int) common.Hash {
	...
	ethTx, err := dynamicFeeTx(suite.ethClient, suite.privateKey, &recipient, value, nil)
	require.NoError(suite.T(), err)

	err = suite.ethClient.SendTransaction(context.Background(), ethTx)
	require.NoError(suite.T(), err)

	suite.PendingTx(ethTx)
...
func (suite *EvmTestSuite) TransferCrossChain(token common.Address, recipient string, amount, fee *big.Int, target string) common.Hash {
	...
	ethTx, err := dynamicFeeTx(suite.ethClient, suite.privateKey, &token, nil, pack)
	require.NoError(suite.T(), err)
...
func (suite *EvmTestSuite) Deposit(wfx common.Address, amount *big.Int) common.Hash {
	pack, err := WFXABI.Pack("deposit")
	require.NoError(suite.T(), err)

	ethTx, err := dynamicFeeTx(suite.ethClient, suite.privateKey, &wfx, amount, pack)
	require.NoError(suite.T(), err)
...
func (suite *EvmTestSuite) Withdraw(wfx, recipient common.Address, value *big.Int) common.Hash {
	pack, err := WFXABI.Pack("withdraw", recipient, value)
	require.NoError(suite.T(), err)

	ethTx, err := dynamicFeeTx(suite.ethClient, suite.privateKey, &wfx, nil, pack)
	require.NoError(suite.T(), err)
...
func (suite *EvmTestSuite) EthBlockHeight() uint64 {
	number, err := suite.ethClient.BlockNumber(context.Background())
	require.NoError(suite.T(), err)
	return number
}

func (suite *EvmTestSuite) CheckIBCChannelState(portId, channelId string) {
	_, err := ibcchanneltypes.NewQueryClient(suite.grpcClient).ChannelClientState(context.Background(), &ibcchanneltypes.QueryChannelClientStateRequest{PortId: portId, ChannelId: channelId})
	require.NoError(suite.T(), err)
}
